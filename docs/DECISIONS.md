...
### D-026 — Append-only source and derivation history for permitted content; narrow age-hold erasure exception

| Field | Value |
|---|---|
| **Decision** | Source snapshots and normalization history records are append-only for permitted content. When content, normalizer version, or registry version changes, a new `normalization_history` record is appended alongside the old record; both are retained. Source-observed text is never altered post-creation. Normal rollback is a revert or fix-forward; history is never rewritten. **Exception — narrow age-hold erasure:** when `hold_age_unknown` is established on a `booth_product`, D-012 erasure requirements override the append-only invariant for prohibited descriptive and body-derived content; body-content excerpts, body-derived content hashes, description fragments, and other prohibited payloads must be purged or irreversibly redacted as specified in [DATA_MODEL.md](DATA_MODEL.md) Section 3.5. This exception is narrow and does not authorize ordinary destructive history rewrites. |
| **Reason** | Immutable history enables: reproducing which source and version produced a value; finding all AI-derived candidates; finding all held or unresolved fields; triggering and tracing reanalysis. Mutable history destroys these capabilities and creates audit liability. The append-only model is also required for reanalysis avoidance: the three-part key (content version, normalizer version, registry version) is meaningless if old records can be overwritten. The age-hold exception is required because D-012 prohibits retention of descriptive content for products with uncertain age classification; the append-only rule governs permitted content only and must not preserve prohibited adult-adjacent payloads. |
| **Rejected alternatives** | Overwrite prior records when content changes — rejected; destroys provenance and prevents reanalysis audit. Delete ended or disappeared product records — rejected; history must be retained for data continuity and reconciliation. Retain prohibited descriptive content under `hold_age_unknown` in the name of append-only — rejected; the append-only rule applies to permitted content only and cannot override D-012 age-hold erasure obligations. |
| **Impact** | `source_snapshot` and `normalization_history` entities are defined in [DATA_MODEL.md](DATA_MODEL.md) Section 8. Destructive operations on permitted records are prohibited at all implementation layers. **Exception — narrow age-hold erasure:** when `hold_age_unknown` is established on a `booth_product`, D-012 age-hold erasure requirements override the append-only invariant for prohibited descriptive and body-derived content. Body-content excerpts, body-derived content hashes, description fragments, and other prohibited descriptive payloads in `source_snapshot` and `normalization_history` records linked to the held product must be purged or irreversibly redacted. Only the minimal non-descriptive tombstone and the controlled redaction event defined in DATA_MODEL.md Section 3.5 remain. This exception is narrow: it applies only to prohibited content when `hold_age_unknown` is active and does not authorize ordinary destructive history rewrites. |
| **Decision date** | 2026-08-01 |
| **Evidence** | Issue #21 acceptance criteria (Section 8); Issue #28 (age-hold erasure exception for append-only invariant); [DATA_COLLECTION_POLICY.md](DATA_COLLECTION_POLICY.md) reanalysis avoidance; [SYSTEM_NORMALIZATION.md](SYSTEM_NORMALIZATION.md) Sections 7.3 and 9.3; D-012 (strict all-ages hold behaviour). |
| **Conditions for revisiting** | Not expected to be revisited. Append-only history for permitted content is a hard audit requirement; the age-hold erasure exception is a hard D-012 compliance requirement. |
