...
| `first_seen_at` | TIMESTAMPTZ | yes | first observation; fallback for New sorting whenever `source_publication_date` does not satisfy `publishable_core_value` |
...
Coverage invariant: every scenario must have at least one `scenario_play_time` row. Zero rows are prohibited so that implicit unknown state cannot arise.
...
Range invariant: when both `min_duration` and `max_duration` satisfy `publishable_core_value`, `min_duration.value <= max_duration.value`. An assignment that violates this invariant must be rejected at write time and must never publish.
...
| `source_url` | TEXT | yes | URL at which this alias text was observed |
| `evidence_type` | TEXT | yes | controlled `EvidenceTypeCode` |
| `evidence_pointer` | TEXT | yes | pointer into the source at `source_url` |
| `first_observed` | TIMESTAMPTZ | yes | earliest confirmed observation of this alias text |
| `last_observed` | TIMESTAMPTZ | yes | most recent confirmed observation of this alias text |
...
Columns: `id`, `canonical_name`, `category`, fixed `provenance = controlled`, `created_at`, Japanese label, optional English label, optional redirect/deprecation fields, and registry version.

`canonical_name` is a machine-safe identifier that is unique within `category`. `provenance` is always `controlled` for registry-managed tags.
...
| `id` | UUID | yes | primary key |
| `scenario_id` | UUID | yes | FK |
| `tag_id` | UUID | yes | required canonical tag reference; null targets fail closed and must not be inserted |
...
Sort/index support is required for discovery order, newest first, last checked, title, seeded-random application ordering, and free-first. Newest first uses `booth_product.source_publication_date.value` when the envelope satisfies `publishable_core_value`, and falls back to `booth_product.first_seen_at` whenever no publishable source date exists, including when the envelope is held, conflicted, rejected, low-confidence, incomplete, or otherwise ineligible. Seeded random is computed at the application/query boundary and does not store a mutable random rank.
