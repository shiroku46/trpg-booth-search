...
Append-only records of old and new normalized results when content or a relevant version changes. History is **never** overwritten or deleted for permitted content.
...
| **All-ages gate** | `parent.all_ages_state.state = known` AND `parent.all_ages_state.value = all_ages_confirmed` AND `parent.all_ages_state.review_state = approved` AND `parent.all_ages_state.confidence ∈ { high, medium }` AND `parent.all_ages_state.source_evidence` is non-empty AND `parent.all_ages_state.conflict_reason = null` AND `parent.all_ages_state.hold_reason` is absent | Excluded from search; low or unresolved confidence, conflict, hold, missing evidence, unreviewed/rejected status, or any non-known state excludes the product from public search; uncertainty is routed through `hold_age_unknown` where applicable |
...
| **Normalization record eligibility gate** | For the published projection of `ruleset_reference`, `compatibility_claim`, and `book_requirement` records: `review_state = approved`; `confidence ∈ { high, medium }`; `source_evidence` is non-empty; `conflict_reason = null`; no blocking `hold_reason`; canonical targets resolved where applicable (see Section 10.3). Records with `review_state ∈ { unreviewed, needs_more_evidence, rejected }`, `confidence ∈ { low, unresolved }`, conflicting evidence, empty evidence, unresolved targets, or a blocking hold are internal and do not enter the public projection. Absence of a blocking hold alone is insufficient; `approved` and `confidence ∈ { high, medium }` are both required. No `review_state = rejected` normalization, alias, ruleset, compatibility, book, or tag record may appear in the public projection for any purpose. | Ineligible normalization record excluded from projection; other independently eligible records for the same scenario are unaffected; the scenario may publish other independently approved fields when remaining gates pass |
...
| Products with `all_ages_state.confidence ∈ { low, unresolved }` | Low or unresolved all-ages confidence does not meet the publication threshold; uncertainty routes through `hold_age_unknown` |
| Products with `all_ages_state.conflict_reason ≠ null` or `all_ages_state.hold_reason` present | Conflicting or held all-ages evidence blocks publication |
| Products with `all_ages_state.state ≠ known` | Non-known all-ages state (including `unknown`, `hold`, `not_applicable`) cannot authorize publication |
...
| `ruleset_reference`, `compatibility_claim`, or `book_requirement` records with `review_state ∈ { unreviewed, needs_more_evidence }` | Unreviewed or incomplete normalization records remain internal; `approved` is required for publication |
| `ruleset_reference`, `compatibility_claim`, or `book_requirement` records with `confidence ∈ { low, unresolved }` | Low or unresolved confidence does not meet the publication threshold for normalization fields |
| `ruleset_reference`, `compatibility_claim`, or `book_requirement` records with empty `source_evidence`, `conflict_reason ≠ null`, or a blocking `hold_reason` | Insufficient evidence, conflicting evidence, or held records remain internal |
| `ruleset_reference` or `compatibility_claim` records with `resolution_state = target_unresolved` | Unresolved canonical targets remain internal; no guessed IDs may be published |
...
| Normalization fields | `ruleset_reference`, `compatibility_claim`, `book_requirement` records that satisfy all eligibility requirements: `review_state = approved`; `confidence ∈ { high, medium }`; `source_evidence` non-empty; `conflict_reason = null`; no blocking `hold_reason`; canonical targets resolved where applicable. Records with `unreviewed`, `needs_more_evidence`, `rejected`, low or unresolved confidence, conflicting evidence, empty evidence, unresolved canonical targets, or a blocking hold remain internal and do not enter the published projection; the scenario may still publish other independently eligible fields. |
...
- No source-observed field (`original_source_text`, `observed_title_text`, `observed_component_wording`, `observed_title`) may be silently overwritten after initial recording. **Exception — narrow age-hold erasure:** when `hold_age_unknown` is established on a `booth_product`, prohibited descriptive source-observed fields must be purged or irreversibly redacted as required by D-012 and as specified in Section 3.5; this exception does not authorize ordinary destructive history rewrites.
- No `source_snapshot` or `normalization_history` record may be deleted or overwritten. **Exception — narrow age-hold erasure:** when `hold_age_unknown` is established, body-content excerpts, body-derived content hashes, description fragments, and other prohibited descriptive payloads in `source_snapshot` and `normalization_history` records linked to the held product must be purged or irreversibly redacted as required by D-012 and as specified in Section 3.5; only the minimal non-descriptive tombstone and the controlled redaction event defined in Section 3.5 remain; this exception does not authorize ordinary destructive history rewrites.
