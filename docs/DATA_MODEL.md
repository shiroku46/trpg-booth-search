...
### 3.6 Classification-Specific Version Invariants

The `classification` field uses `EvidencedValue<ProductClassCode>`. To ensure that every product reclassification event can populate all required fields of a `normalization_history` record without null version values, the following invariants apply exclusively to `booth_product.classification`:

- **`content_version`** is always non-null (required by the base `EvidencedValue` contract).
- **`normalizer_version`** must always be non-null for every `classification` result. When an automated classifier pipeline produces the result, use the classifier pipeline version string. When no automated classifier applies (e.g., the classification is determined by a deterministic rule or manual review), use an explicit sentinel string such as `deterministic_v<N>` or `manual_v<N>` where `<N>` identifies the version of the rule set or manual review protocol in effect at the time.
- **`registry_version`** must always be non-null for every `classification` result. When a canonical registry lookup is performed as part of classification, use the registry snapshot version. When classification does not consult the canonical registry (which is typical for `ProductClassCode` values), use an explicit empty/minimal registry version identifier such as `registry_empty_v0` or `registry_not_consulted_v<N>`.

**Reclassification and `normalization_history`:** When a `booth_product.classification` value changes — for example when a product is reclassified from `material_only` to `scenario_single` following new evidence — a `normalization_history` record must be appended with `target_type = booth_product` and `target_id = booth_product.id`. Because `content_version`, `normalizer_version`, and `registry_version` on every `classification` result are always non-null under this invariant, the six version fields of the resulting `normalization_history` record (`content_version_old`, `content_version_new`, `normalizer_version_old`, `normalizer_version_new`, `registry_version_old`, `registry_version_new`) are always populated with non-null values. This satisfies the non-null version constraint of Section 8.2 for all product classification transitions without exception.
...
| `registry_version` | string | Required when `provenance = derived`; always non-null | Registry snapshot version consulted when producing this derived tag assignment; always recorded for every derived tag even when no separate canonical-resolution step was performed or the registry is empty/minimal; use an explicit non-null empty/minimal registry version identifier (e.g., `registry_empty_v0`) when no registry entries exist; a registry-version change reproducibly invalidates the derived-tag result and triggers reanalysis; never null |
...
- `provenance = source` tags may publish without explicit approval only when ALL of: `conflict_state = clear`, `hold_reason` absent, `spoiler_suspect = false`, `source_evidence` non-empty, and `review_state ∈ { unreviewed, approved }`. `review_state ∈ { rejected, needs_more_evidence }` must not publish regardless of other fields.
- **Reanalysis key (derived tags):** The complete derived-tag reanalysis key is the three-tuple (`content_version`, `classifier_version`, `registry_version`); all three are always non-null for derived tags. Reanalysis is skipped only when all three are unchanged since the last analysis. A change to any one — including a registry-version change — invalidates the key and requires a new `normalization_history` entry with `target_type = scenario_tag`. Because `registry_version` is always non-null, `normalization_history` records for derived-tag transitions always carry non-null old and new registry versions.
...
| **Source `scenario_tag` publication gate** | For `provenance = source` tags, ALL of: `spoiler_suspect = false`; `conflict_state = clear`; `hold_reason` absent; `source_evidence` non-empty; `review_state ∈ { unreviewed, approved }`. Records with `spoiler_suspect = true`, `conflict_state ≠ clear`, any `hold_reason`, empty `source_evidence`, or `review_state ∈ { rejected, needs_more_evidence }` must not appear. AI-derived tags (`is_ai_derived = true`) continue to require `review_state = approved` under the derived-tag gate regardless of `provenance`. | Ineligible source `scenario_tag` excluded; other eligible source tags for the same scenario are unaffected |
...
| Source `scenario_tag` records (`provenance = source`) with `spoiler_suspect = true`, `conflict_state ≠ clear`, any `hold_reason`, empty `source_evidence`, or `review_state ∈ { rejected, needs_more_evidence }` | Source tags that are spoiler-suspect, conflict-bearing, held, evidence-empty, rejected, or needs-more-evidence are excluded from public results; use the actual fields `conflict_state`, `hold_reason`, `review_state`, `source_evidence`, and `spoiler_suspect` as defined on `scenario_tag` |
...
| Published source tags | Source `scenario_tag` records (`provenance = source`) with `spoiler_suspect = false`, `conflict_state = clear`, `hold_reason` absent, `source_evidence` non-empty, and `review_state ∈ { unreviewed, approved }`; `review_state ∈ { rejected, needs_more_evidence }` excluded |
