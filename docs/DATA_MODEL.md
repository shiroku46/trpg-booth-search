...
| **`book_requirement` publication gate** | ALL of: `review_state = approved`; `book_identity_state = resolved`; `conflict_status = clear`; `source_evidence` non-empty. Note: `book_requirement` has no `confidence` field and no `conflict_reason` field; `book_identity_state` and `conflict_status` are the applicable status and conflict indicators. Records with `review_state ∈ { unreviewed, needs_more_evidence, rejected }`, `book_identity_state ∈ { hold_book_conflict, unresolved }`, `conflict_status = hold_book_conflict`, or empty `source_evidence` must not appear. | Ineligible `book_requirement` excluded; other approved records for the same scenario are unaffected |
...
| `book_requirement` (normalized) | Records with `review_state = approved`, `book_identity_state = resolved`, `conflict_status = clear`, non-empty `source_evidence`; `book_requirement` has no `confidence` field and no `conflict_reason` field — the applicable indicators are `book_identity_state` and `conflict_status`; `unreviewed`, `needs_more_evidence`, `rejected`, `hold_book_conflict`, `unresolved`, or evidence-empty records excluded |
